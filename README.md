...
* Т.к. при большом количестве данных скрипт будет выполнятся достаточно долго нужно увеличить лимиты таймаута для БД:
  * На 6 тысячах записей в таблице clients один тест проходит примерно за 45 секунд
  * Для mysql это `interactive_timeout` и `wait_timeout` можно поставить час - 3600
